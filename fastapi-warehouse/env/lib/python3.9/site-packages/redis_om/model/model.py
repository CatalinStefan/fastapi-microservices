...
class RedisModel(BaseModel, abc.ABC, metaclass=ModelMeta):
    ...
    @classmethod
    def delete(cls, pk: Any) -> int:
        """Delete data at this key."""
        return cls.db().delete(cls.make_primary_key(pk))
    ...
    def expire(self, num_seconds: int, pipeline: Optional[Pipeline] = None):
        if pipeline is None:
            db = self.db()
        else:
            db = pipeline
        ...
        db.expire(self.make_primary_key(self.pk), num_seconds)
